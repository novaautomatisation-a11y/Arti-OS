...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (company?.id) {
      ...
      // Si pas de company, arrêter le chargement
      setLoading(false)
    }
  }, [company?.id])
    ...
    return (
      <div className="flex items-center justify-center min-h-96">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Aucune entreprise associée</h2>
          <p className="text-gray-400 mb-6">
            Votre profil n'est pas encore associé à une entreprise.
            Veuillez contacter l'administrateur ou configurer votre entreprise dans les paramètres.
          </p>
          <button
            onClick={() => router.push('/app/settings')}
            className="px-6 py-3 bg-warning text-dark font-semibold rounded-lg hover:bg-warning-dark transition-colors"
          >
